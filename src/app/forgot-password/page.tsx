...
import { Mail, ArrowRight, ArrowLeft, ShieldCheck, Sparkles, Zap, Smartphone, ChevronRight } from "lucide-react";
          ...
          >
                  ...
                  <div className="glass-card p-10 border-white/60 bg-white/20 backdrop-blur-xl space-y-8 shadow-2xl">
                    <form className="space-y-6" onSubmit={handleSubmit}>
                      <div className="space-y-2">
                        <label htmlFor="email" className="text-[10px] font-black text-taja-primary uppercase tracking-[0.2em] ml-1">
                          Recovery Email
                        </label>
                ...
                >
                  ...
                  <div className="space-y-3">
                    <h2 className="text-3xl font-black text-taja-secondary tracking-tighter">
                      Link <span className="text-taja-primary">Deployed</span>
                    </h2>
                    <p className="text-sm font-medium text-gray-500 tracking-wide leading-relaxed">
                      Check your communications at <br />
                      <span className="text-taja-secondary font-black">{email}</span>
                    </p>
                  </div>
            ...
            <motion.div variants={itemVariants} className="pt-8 border-t border-gray-100">
              <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-[0.3em] text-gray-400">
                <Link href="/" className="hover:text-taja-primary transition-colors">Home</Link>
                <div className="h-1 w-1 rounded-full bg-gray-200" />
                <Link href="/contact" className="hover:text-taja-primary transition-colors">Support</Link>
                <div className="h-1 w-1 rounded-full bg-gray-200" />
                <Link href="/help" className="hover:text-taja-primary transition-colors">Help Center</Link>
              </div>
            </motion.div>
          </motion.div>
        </div>
        ...
        <div className="hidden md:flex relative flex-col items-center justify-center p-12 overflow-hidden bg-white border-l border-gray-50">
          ...
          <div className="relative z-10 w-full max-w-sm space-y-12">
            <div className="space-y-4 text-center lg:text-left">
              ...
              >
                <h3 className="text-4xl font-black text-taja-secondary tracking-tighter leading-none">
                  Account Recovery <br />
                  <span className="text-transparent bg-clip-text bg-gradient-taja">Made Simple.</span>
                </h3>
              </motion.div>
              <motion.p
                initial={{ opacity: 0, x: 30 }}
              ...
              >
                We'll help you get back to your shop in no time. Your security is our top priority, but the process shouldn't be a headache.
              </motion.p>
            </div>
            ...
            <div className="space-y-4">
              {[
                { icon: ShieldCheck, title: "Smart Validation", desc: "Proving it's you, without the friction." },
                { icon: Zap, title: "Instant Sync", desc: "Get back to selling immediately once verified." },
                { icon: Smartphone, title: "Recognized Hubs", desc: "We remember your trusted, verified devices." },
                { icon: Sparkles, title: "Neural Lock", desc: "AI-monitored recovery for elite security." },
              ].map((feature, i) => (
                <motion.div
                  key={i}
                ...
                >
                  ...
                  <div>
                    <h4 className="text-sm font-black text-taja-secondary tracking-tight mb-0.5">{feature.title}</h4>
              ...
              <Link href="/help">
                <div className="glass-card bg-white/50 border-gray-100 p-6 flex items-center justify-between group hover:border-taja-primary/40 transition-all duration-500 rounded-[2rem]">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="text-[10px] font-black text-taja-secondary uppercase tracking-widest">Still restricted?</h4>
                      <p className="text-[10px] text-gray-400 font-medium">Connect with our security task force.</p>
                    </div>
                  </div>
          ...
          <div className="absolute bottom-8 left-12 flex items-center gap-3">
            <div className="flex -space-x-2">
              {[1, 2, 3].map(n => (
                <div key={n} className="h-8 w-8 rounded-full border-2 border-white bg-gray-100 shadow-sm overflow-hidden">
                  <img src={`https://i.pravatar.cc/100?img=${n + 25}`} alt="User" className="h-full w-full object-cover opacity-80" />
                </div>
              ))}
            </div>
            <p className="text-[10px] font-black text-gray-300 uppercase tracking-widest">Securing Top Shops</p>
          </div>
        </div>
      </div>
    </div>
