...
export default function AdminOverviewBannedUsersPage() {
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState("");
  const [appliedSearch, setAppliedSearch] = useState("");
  const [page, setPage] = useState(1);
  const [data, setData] = useState<{
    summary: { banned: number };
    users: any[];
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await api(`/api/admin/overview/banned-users?${q}`);
        if (cancelled) return;
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto">
      <OverviewHeader
        title="Banned users"
        subtitle="Accounts with account status banned."
      />
          ...
          {loading && !data ? (
            ...
            <>
              <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
                <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Banned</span>
                <span className="text-xl font-black text-slate-900">
                  {(data?.summary.banned ?? 0).toLocaleString()}
                </span>
              </div>
